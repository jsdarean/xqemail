/**
 * 我的待办页面脚本
 * 展示所有涉及开发的工作量已评估、但至少有一个开发单号未录入的需求。
 */
...
function sortData(data) {
    ...
    reqIds.sort((a, b) => {
        const dateA = String(groups[a][0].propose_date || '9999').substring(0, 10);
        const dateB = String(groups[b][0].propose_date || '9999').substring(0, 10);
...
async function loadData() {
    ...
    try {
        const response = await fetchWithAuth('/api/emails/todo');
...
function updateSummary() {
    ...
    if (reqIds.length === 0) {
        summary.innerHTML = '当前没有待录入开发单号的需求。';
        return;
    }
    summary.innerHTML = '当前您有 <strong>' + reqIds.length + '</strong> 个需求已完成了工作量全部评估，请及时转开发单并录入开发单号。';
}
...
function renderGroupedTable() {
    ...
    // 排序
    reqIds.sort((a, b) => {
        const dateA = String(groupedData[a][0].propose_date || '9999').substring(0, 10);
        const dateB = String(groupedData[b][0].propose_date || '9999').substring(0, 10);
        if (dateA !== dateB) return dateB.localeCompare(dateA);
        return String(a || '').localeCompare(String(b || ''));
    });
